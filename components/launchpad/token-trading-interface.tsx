...
export function TokenTradingInterface({
  tokenPrincipal,
}: TokenTradingInterfaceProps) {
  ...
  const fetchData = async () => {
    try {
      const [launchpadAddress, launchpadName] =
        CONTRACTS.TESTNET.LAUNCHPAD.split(".");
  ...
  // Calculations
  const currentPriceInUstx = tokenState
    ? 1000 + tokenState.supply / 1000
    : 1000;
  ...
  if (!tokenState) {
    return (
      <div className="p-10 text-center">Token not found or not registered.</div>
    );
  }

  const progress = Math.min(
    (tokenState.supply / tokenState.targetSupply) * 100,
    100,
  );
    ...
    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
      ...
      <div className="md:col-span-2 space-y-6">
            ...
            <div className="flex justify-between items-start">
              <div>
                <CardTitle className="flex items-center gap-2">
                  ...
                  {tokenState.metadataUrl &&
                    tokenState.metadataUrl.includes("github") && (
                      <a
                        href={tokenState.metadataUrl}
                        target="_blank"
                ...
                <CardDescription>
                  Created by {tokenState.creator}
                </CardDescription>
              </div>
              <div className="text-right">
                <Badge
                  variant={blocksRemaining > 0 ? "default" : "destructive"}
          ...
          <CardContent className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="p-4 bg-muted/50 rounded-lg">
              ...
              <div className="text-xl font-bold">
                {currentPriceStx.toFixed(6)} STX
              </div>
            </div>
            <div className="p-4 bg-muted/50 rounded-lg">
              <div className="text-sm text-muted-foreground">Market Cap</div>
              <div className="text-xl font-bold">
                {(currentPriceStx * tokenState.supply).toLocaleString()} STX
              </div>
            </div>
            <div className="p-4 bg-muted/50 rounded-lg">
              <div className="text-sm text-muted-foreground">Supply</div>
              <div className="text-xl font-bold">
                {tokenState.supply.toLocaleString()}
              </div>
            </div>
            <div className="p-4 bg-muted/50 rounded-lg">
              <div className="text-sm text-muted-foreground">Reserve</div>
              <div className="text-xl font-bold">
                {(tokenState.stxReserve / 1000000).toLocaleString()} STX
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Funds are secured by Bitcoin finality. Graduation triggers
              liquidity lock.
            </CardDescription>
          </CardHeader>
          <CardContent>
            ...
            <div className="mt-2 flex justify-between text-sm text-muted-foreground">
              ...
              <span>
                Target: {tokenState.targetSupply.toLocaleString()} Tokens
              </span>
            </div>
          </CardContent>
        </Card>
          ...
          <Card className="border-indigo-500/20 bg-indigo-500/5">
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                <Progress
                  value={(vesting.claimed / vesting.total) * 100}
                  className="h-2"
                />
      ...
      <div>
        <Card className="h-full border-primary/20">
          <CardHeader>
            <CardTitle>
              Trade {tokenState.active ? "" : "(Graduated)"}
            </CardTitle>
            ...
            <Tabs defaultValue="buy" className="w-full">
              ...
              <TabsContent value="buy" className="space-y-4">
                ...
                <div className="text-sm text-muted-foreground">
                  Est. Tokens:{" "}
                  {amount
                    ? (Number(amount) / currentPriceStx).toFixed(2)
                    : "0.00"}
                </div>
                <Button
                  className="w-full bg-green-600 hover:bg-green-700"
                  onClick={handleBuy}
                  disabled={isTrading || !tokenState.active}
                >
                  {isTrading ? (
                    <Loader2 className="animate-spin" />
                  ) : (
              ...
              <TabsContent value="sell" className="space-y-4">
                ...
                <div className="text-sm text-muted-foreground">
                  Est. STX:{" "}
                  {amount
                    ? (Number(amount) * currentPriceStx).toFixed(6)
                    : "0.00"}
                </div>
                <Button
