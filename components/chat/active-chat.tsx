...
function MessageBubble({
  ...
  return (
    <div
      className={`max-w-[75%] rounded-2xl px-5 py-3 shadow-sm relative group ${
        isMe
    ...
    >
      ...
      <p
        className={`text-[10px] mt-1.5 text-right ${isMe ? "text-white/70" : "text-muted-foreground"}`}
      >
...
export function ActiveChat({ conversationId }: ActiveChatProps) {
    ...
    const fetchData = async () => {
      try {
        ...
        const resDetails = await fetch(
          `${process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:3001"}/api/chat/${conversationId}/details`,
        );
        if (resDetails.ok) {
        ...
        const resHistory = await fetch(
          `${process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:3001"}/api/chat/${conversationId}/messages`,
        );
        if (resHistory.ok) {
  ...
  const acceptChat = async () => {
    try {
      await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:3001"}/api/chat/${conversationId}/accept`,
        { method: "POST" },
      );
  ...
  const otherMember = conversationDetails?.participants?.find(
    (p: any) => p.address !== user?.address,
  );
  const displayName =
    otherMember?.name || otherMember?.address || conversationId;
  ...
  useEffect(() => {
    ...
    const otherMessages = currentMessages.filter(
      (m) =>
        m.senderAddress === conversationId || m.senderAddress !== user?.address,
    );
    for (const m of otherMessages) {
      if (m.content.startsWith(HANDSHAKE_PREFIX)) {
        try {
          const payload = JSON.parse(
            m.content.substring(HANDSHAKE_PREFIX.length),
          );
  ...
  const visibleMessages = currentMessages.filter(
    (m) => !m.content.startsWith(HANDSHAKE_PREFIX),
  );
      ...
      <header className="flex items-center justify-between p-4 border-b border-white/5 bg-background/60 backdrop-blur-xl sticky top-0 z-10">
          ...
          <div>
            <h3 className="font-semibold text-sm flex items-center gap-2">
              ...
              {recipientPublicKey ? (
                ...
                <span title="Not Encrypted" className="text-amber-500">
            ...
            <div className="flex items-center gap-1.5">
              <span className="relative flex h-2 w-2">
                <span
                  className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${conversationDetails?.status === "REQUESTED" ? "bg-amber-400" : "bg-green-400"}`}
                ></span>
                <span
                  className={`relative inline-flex rounded-full h-2 w-2 ${conversationDetails?.status === "REQUESTED" ? "bg-amber-500" : "bg-green-500"}`}
                ></span>
              </span>
        ...
        {conversationDetails?.status === "REQUESTED" && (
          <Button
            size="sm"
            onClick={acceptChat}
            className="bg-primary hover:bg-primary/90 text-white ml-auto mr-4"
          >
            Accept Request
          </Button>
        )}
        ...
        {visibleMessages.length === 0 ? (
          ...
          <Virtuoso
            ...
            itemContent={(index, msg) => {
              ...
              return (
                <div
                  className={`flex ${isMe ? "justify-end" : "justify-start"} pb-6 px-1`}
                >
                  <MessageBubble
                    msg={msg}
      ...
      <div className="p-4 bg-transparent">
        <div className="max-w-3xl mx-auto">
          <form
            ...
            className="flex gap-2 items-end glass-card p-2 rounded-2xl focus-within:ring-1 focus-within:ring-primary/50 transition-all shadow-lg"
          >
            <Button
              type="button"
              size="icon"
              variant="ghost"
              className="text-muted-foreground hover:text-primary h-10 w-10 shrink-0 rounded-xl"
            >
            ...
            <div className="flex gap-1">
              ...
              <Button
                type="submit"
                size="icon"
                className={`h-10 w-10 shrink-0 rounded-xl shadow-md text-white ${recipientPublicKey ? "bg-green-600 hover:bg-green-700" : "bg-primary hover:bg-primary/90"}`}
